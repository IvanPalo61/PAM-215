...
import UsuarioController from './controllers/UsuarioController'; // Ajusta la ruta si es necesario
...
export default function App() {
  ...
  const cargarUsuarios = useCallback(async () => {
    try {
      ...
      console.log(`${(data && data.length) || 0} usuarios cargados`);
    } catch (error) {
  ...
  // Inicializar y registrar listener
  useEffect(() => {
    const init = async () => {
  ...
  const handleAgregar = async () => {
    ...
    if (!nombre || !nombre.trim()) {
      Alert.alert('Aviso', 'Ingresa un nombre');
      return;
    }
    try {
      setGuardando(true);
      ...
      Alert.alert(
        'Usuario Creado',
        `${usuarioCreado?.nombre || 'Usuario'} guardado con ID: ${usuarioCreado?.id ?? 'N/A'}`
      );
      setNombre('');
      // Recargar (el observer también puede hacerlo)
      await cargarUsuarios();
    } catch (error) {
      Alert.alert('Error', error?.message || String(error));
    } finally {
  ...
  // Render de cada usuario para FlatList
  const renderUsuario = ({ item, index }) => (
    <View style={styles.userItem}>
      ...
      <View style={styles.userInfo}>
        ...
        <Text style={styles.userId}>ID: {String(item.id)}</Text>
